...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn("role_id", "uuid", (col) =>
      col.references("organization.role.id").notNull(),
    )
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await db.schema.dropTable("organization.role_permission").execute();
  await db.schema.dropTable("organization.permission").execute();
  await db.schema.dropTable("organization.role").execute();
  await db.schema.dropTable("organization.member").execute();
  await db.schema.dropTable("organization.organization").execute();
