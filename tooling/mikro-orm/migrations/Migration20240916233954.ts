...
export class Migration20240916233954 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql('alter table "collaboration"."workspace_threads" drop column "name";');
  }

  override async down(): Promise<void> {
    this.addSql('alter table "collaboration"."member" drop constraint "member_workspace_id_foreign";');

    this.addSql('drop table if exists "identity"."user" cascade;');

    this.addSql('drop table if exists "collaboration"."workspace" cascade;');

    this.addSql('drop table if exists "collaboration"."member" cascade;');

    this.addSql('alter table "collaboration"."workspace_threads" add column "name" varchar(255) not null;');
